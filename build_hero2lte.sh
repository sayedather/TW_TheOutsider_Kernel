...
herolte)
	...
	can|duos|eur|xx)
		...
		;;
	kor|ktt|lgt|skt)
		KERNEL_DEFCONFIG=kylothow-$MODEL-kor_defconfig
		;;
	*)
...
FUNC_BUILD_ZIP()
{
	...
	herolte)
		case $VARIANT in
		can|duos|eur|xx)
			mv -f $RDIR/ramdisk/SM-G930F/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		ktt)
			mv -f $RDIR/ramdisk/SM-G930K/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		lgt)
			mv -f $RDIR/ramdisk/SM-G930L/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		skt)
			mv -f $RDIR/ramdisk/SM-G930S/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		*)
			echo "Unknown variant: $VARIANT"
			exit 1
	...
	hero2lte)
		case $VARIANT in
		can|duos|eur|xx)
			mv -f $RDIR/ramdisk/SM-G935F/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		ktt)
			mv -f $RDIR/ramdisk/SM-G935K/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		lgt)
			mv -f $RDIR/ramdisk/SM-G935L/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		skt)
			mv -f $RDIR/ramdisk/SM-G935S/image-new.img $RDIR/build/$MODEL/$MODEL-$VARIANT.img
			;;
		*)
			echo "Unknown variant: $VARIANT"
			exit 1
